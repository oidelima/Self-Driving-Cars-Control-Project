function z = generatePart1(nsteps, x0, subdivision_num, gradient_prec)
load TestTrack.mat
global n dt leftBound rightBound F_max centerLine precompFlag gradient_precision curr_x0
...
gradient_precision = gradient_prec;
...
function [g,h,dg,dh]=nonlcon(z)
    global n dt leftBound rightBound F_max gradient_precision curr_x0
    ...
    dg_dxy = torGradient(@inequalityConstraintTrack, [x,y]', gradient_precision(1), leftBound, rightBound)';
...
function F = F_total(x,u)
    global precompFlag F_totaldata F_totalvals
    if precompFlag == 1
        %F = griddatan(F_totaldata, F_totalvals, [x,u]);
    end
    % States: z = [x; u; y; v; psi; r]
...
function J = J1(x,u,y,v,psi_,r,delta_f,F_x)
    global precompFlag J1data J1vals
    if precompFlag == 1
        %J = griddatan(J1data, J1vals, [x,u,y,v,psi_,r,delta_f,F_x]);
    end
...
function J = J1sat(x,u,y,v,psi_,r,delta_f,F_x)
    global precompFlag J1satdata J1satvals
    if precompFlag == 1
        %J = griddatan(J1satdata, J1satvals, [x,u,y,v,psi_,r,delta_f,F_x]);
    end
    J = [0,cos(psi_), 0,-sin(psi_), - v*cos(psi_) - u*sin(psi_),0;
...
function J = J2(x,u,y,v,psi_,r,delta_f,F_x)
    global precompFlag J2data J2vals
    if precompFlag == 1
        %J = griddatan(J2data, J2vals, [x,u,y,v,psi_,r,delta_f,F_x]);
    end
    J = [0,0;
...
function J = J2sat(x,u,y,v,psi_,r,delta_f,F_x)
    global precompFlag J2satdata J2satvals
    if precompFlag == 1
        %J = griddatan(J2satdata, J2satvals, [x,u,y,v,psi_,r,delta_f,F_x]);
    end
    J = [0,0;
